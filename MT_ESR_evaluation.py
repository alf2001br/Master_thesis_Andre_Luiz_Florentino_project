...
class ESR_evaluation:
    ...
    def __init__(self, audio: list, classifier: str, path_models: str, path_arrays: str):
        ...
        # Load the classifier SUPPORT VECTOR MACHINE (SVC) with the highest accuracy
    ...
    def _val_predict(self):
        ...
        if self.classifier == 'LR':
            ...
            predict_val = np.argmax(self.model_ANN.predict(np.array(self.features_vector).reshape(1, -1)), axis=1)
            self.predictions.append(predict_val[0])
            print(self.nom_classes[predict_val[0]])

        elif self.classifier == 'CNN1D':
            predict_val = np.argmax(
                self.model_CNN_1D.predict(np.array(self.features_vector).reshape(1, -1)[..., np.newaxis]), axis=1)
            self.predictions.append(predict_val[0])
            print(self.nom_classes[predict_val[0]])

        elif self.classifier == 'CNN2D':
            predict_val = np.argmax(self.model_CNN_2D.predict(self.features_agg), axis=1)
            self.predictions.append(predict_val[0])
            print(self.nom_classes[predict_val[0]])
